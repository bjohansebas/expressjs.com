...
For details, see [Application settings](/en/5x/api#application-settings).
:::
...
:::
  ...
  `env`
      ...
      see <a href="/advanced/best-practice-performance#env">Production best practices: performance and reliability</a>.
    </td>
  <td markdown="1">
...
###### Options for `trust proxy` setting
  ...
  <p markdown="1">
  Read [Express behind proxies](/guide/behind-proxies) for more
  information.
  </p>
...
When the [`trust proxy` setting](api#application-settings)
...
When the [`trust proxy` setting](/5x/api#options-for-trust-proxy-setting)
does not evaluate to `false`, this property will instead get the value
from the `X-Forwarded-Host` header field. This header can be set by
the client or by the proxy.
...
When the [`trust proxy` setting](/5x/api#options-for-trust-proxy-setting) does not evaluate to `false`,
...
If you need to make changes to a key in `req.params`, use the [app.param](/5x/api#appparamname-callback) handler. Changes are applicable only to [parameters](/guide/routing#route-parameters) already defined in the route path.
...
:::note
When called from a middleware, the mount point is not included in `req.path`. See [app.use()](/5x/api#appusepath-callback--callback) for more details.
:::
...
using [app.set](/5x/api#appsetname-value).
...
Later, you may access this value through the [req.signedCookies](#request.signedCookies) object.
