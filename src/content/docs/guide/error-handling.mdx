...
Other error handling middleware can be found at [Express middleware](/resources/middleware.html).
...
<div class="doc-box doc-info" markdown="1">
  ...
  complete and in what state. `next(err)` will skip all remaining handlers in
  the chain except for those that are set up to handle errors as described
  above.
</div>
