...
:::note
  When configuring the `trust proxy` setting, it is important to understand the
  exact setup of the reverse proxy. Since this setting will trust values
  provided in the request, it is important that the combination of the setting
...
<ul>
  <li markdown="1">
    The value of [req.hostname](/api.html#req.hostname) is derived from the
  ...
  <li markdown="1">
    ...
    [req.protocol](/api.html#req.protocol).
  </li>
  <li markdown="1">
    The [req.ip](/api.html#req.ip) and [req.ips](/api.html#req.ips) values are
