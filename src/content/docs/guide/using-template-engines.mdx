...
The [Express application generator](/starter/generator) uses [Jade](https://www.npmjs.com/package/jade) as its default, but it also supports several others.
...
:::caution
  **Note**: Jade has been renamed to [Pug](https://www.npmjs.com/package/pug).
  You can continue to use Jade in your app, and it will work just fine. However
  if you want the latest updates to the template engine, you must replace Jade
  with Pug in your app.
:::

To render template files, set the following [application setting properties](/4x/api.html#app.set), set in `app.js` in the default app created by the generator:
...
<PackageManagerCommand npm="install pug --save" pnpm="add pug" bun="add pug"/>
...
["Developing template engines for Express"](/advanced/developing-template-engines.html).
