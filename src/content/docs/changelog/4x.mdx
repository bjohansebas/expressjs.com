...
<ul>
  <li markdown="1" class="changelog-item">
  The [`app.get()` method](/4x/api#application.get) and the [`app.set()` method](/4x/api#appsetname-value) now ignores properties directly on `Object.prototype` when getting a setting value.
  </li>

{" "}

<li markdown="1" class="changelog-item">
  The [`res.cookie()` method](/4x/api#response.cookie) now accepts a "priority"
...
<li markdown="1" class="changelog-item">
  The [`res.cookie()` method](/4x/api#response.cookie) now rejects an Invalid
...
<li markdown="1" class="changelog-item">
  The [`res.cookie()` method](/4x/api#response.cookie) now works when `null` or
...
<li markdown="1" class="changelog-item">
  The [`res.download()` method](/4x/api#response.download) now accepts a "root"
...
<li markdown="1" class="changelog-item">
  The [`res.download()` method](/4x/api#response.download) can be supplied with
...
<li markdown="1" class="changelog-item">
  The [`res.format()` method](/4x/api#response.format) now invokes the provided
...
<li markdown="1" class="changelog-item">
  The [`res.send()` method](/4x/api#response.send) will not attempt to send a
...
<li markdown="1" class="changelog-item">
  The [`res.download()` method](/4x/api#response.download) now accepts an
...
<li markdown="1" class="changelog-item">
  The [`res.location()` method](/4x/api#response.location) and [`res.redirect()`
  method](/4x/api#response.redirect) will now URL-encode the URL string, if it
...
<li markdown="1" class="changelog-item">
  The performance of the [`res.json()` method](/4x/api#response.json) and
  [`res.jsonp()` method](/4x/api#response.jsonp) have been improved in the
  common cases.
</li>

<li markdown="1" class="changelog-item">
  The [jshttp/cookie module](https://www.npmjs.com/package/cookie) (in addition to a number of other improvements) has been updated and now the [`res.cookie()` method](/4x/api#response.cookie) supports the `sameSite` option to let you specify the [SameSite cookie attribute](https://tools.ietf.org/html/draft-west-first-party-cookies-07).  NOTE: This attribute has not yet been fully standardized, may change in the future, and many clients may ignore it.
...
<li markdown="1" class="changelog-item">
  ...
  <li markdown="1" class="changelog-item">
  The [`req.range()` method](/4x/api#request.range) options object now supports a `combine` option (`false` by default), which when `true`, combines overlapping and adjacent ranges and returns them as if they were specified that way in the header.
  </li>
</ul>
