...
:::note
  ...
  Procedures](/en/resources/contributing#security-policies-and-procedures).
:::
...
<PackageManagerCommand npm="install --save helmet" pnpm="add helmet" bun="add helmet"/>
...
:::note
  Disabling the `X-Powered-By header` does not prevent a
  sophisticated attacker from determining that an app is running Express. It may
  discourage a casual exploit, but there are other ways to determine an app is
  running Express.
:::
...
<PackageManagerCommand npm="audit"/>
...
<PackageManagerCommand npm="install -g snyk" pnpm="install -g snyk" bun="install -g snyk"/>
