...
It serves static files and is based on  [serve-static](https://github.com/expressjs/serve-static/blob/master/README.md).

:::note
For best results, [use a reverse proxy](/advanced/best-practice-performance#use-a-reverse-proxy) cache to improve performance of serving static assets.
:::
...
| `etag`        | Enable or disable etag generation <br/><Aside type='note'>`express.static` always sends weak ETags.</Aside> | Boolean | `true` |
...
This is a built-in middleware function in Express. It parses incoming requests
with urlencoded payloads and is based on [body-parser](https://github.com/expressjs/body-parser/blob/master/README.md).
...
and will be available in templates rendered with [res.render](#response.render).
...
in contrast with [res.locals](#response.locals) properties that
are valid only for the lifetime of the request.

You can access local variables in templates rendered within the application.
This is useful for providing helper functions to templates, as well as application-level data.
Local variables are available in middleware via `req.app.locals` (see [req.app](#request.app))
...
It is similar to the [baseUrl](#request.baseUrl) property of the `req` object, except `req.baseUrl`
...
For details, see [Application settings](/en/4x/api#appsetname-valuetings.table).
:::
...
it is usually better to use [req.baseUrl](#request.baseUrl) to get the canonical path of the app.
...
that is an object containing local variables for the view. It is like [res.render()](#response.render),
...
:::
  ...
  <td>String</td>
  <td markdown="1">
  Environment mode. Be sure to set to "production" in a production environment; see [Production best practices: performance and reliability](/advanced/best-practice-performance#env).
  </td>
  <td markdown="1">
...
The [express.static](#expressstaticroot-options) middleware ignores these settings.
:::

The ETag functionality is implemented using the [etag](https://www.npmjs.org/package/etag) package.
...
The `req.baseUrl` property is similar to the [mountpath](#appmountpath) property of the `app` object,
...
as [`express.json()`](#expressjsonoptions) or [`express.urlencoded()`](#expressurlencodedoptions).
...
If the cookie has been signed, you have to use [req.signedCookies](#request.signedCookies).
...
When the [`trust proxy` setting](/4x/api#options-for-trust-proxy-setting)
...
When the [`trust proxy` setting](/4x/api#options-for-trust-proxy-setting) does not evaluate to `false`,
...
the "mounting" feature of [app.use()](#appusepath-callback--callback) will rewrite `req.url` to strip the mount point.
...
If you need to make changes to a key in `req.params`, use the [app.param](/4x/api#application.param) handler. Changes are applicable only to [parameters](/guide/routing#route-parameters) already defined in the route path.
...
:::note
When called from a middleware, the mount point is not included in `req.path`. See [app.use()](/4x/api#application.use) for more details.
:::
...
When the [`trust proxy` setting](#options-for-trust-proxy-setting) does not evaluate to `false`,
...
Check out the [query parser application setting](#application-settings) documentation for other customization options.
...
For more information, see [req.fresh](#request.fresh).
...
using [app.set](/4x/api#appsetname-value).
...
Body-parsing middleware must be loaded for `req.param()` to work predictably. Refer [req.body](#request.body) for details.
:::
...
`res.app` is identical to the [req.app](#request.app) property in the request object.
...
Use this property to set variables accessible in templates rendered with [res.render](#response.render).
...
Later you may access this value through the [req.signedCookie](#request.signedCookies) object.
...
Clears the cookie specified by `name`. For details about the `options` object, see [res.cookie()](#response.cookie).

:::caution
Web browsers and other compliant clients will only clear the cookie if the given
`options` is identical to those given to [res.cookie()](#response.cookie), excluding
`expires` and `maxAge`.
:::
...
Use to quickly end the response without any data. If you need to respond with data, instead use methods such as [res.send()](#response.send) and [res.json()](#response.json).
...
It uses [req.accepts()](#request.accepts) to select a handler for the request, based on the acceptable
...
<a href="#application-settings">jsonp callback name</a> setting.
...
[app.use()](#appusepath-callback--callback) or as the argument to another router's  [use()](#routerusepath-function--function) method.

The top-level `express` object has a [Router()](#expressrouteroptions) method that creates a new `router` object.
...
This method is similar to [app.use()](#appusepath-callback--callback). A simple example and use case is described below.
See [app.use()](#appusepath-callback--callback) for more information.
