...
Read [Express behind proxies](/guide/behind-proxies.html) for more information.
...
If you need to make changes to a key in `req.params`, use the [app.param](/4x/api.html#app.param) handler. Changes are applicable only to [parameters](/guide/routing.html#route-parameters) already defined in the route path.
...
using [app.set](/4x/api.html#app.set).
