...
and will be available in templates rendered with [res.render](#resrenderview--locals--callback).
...
in contrast with [res.locals](#reslocals) properties that
are valid only for the lifetime of the request.

You can access local variables in templates rendered within the application.
This is useful for providing helper functions to templates, as well as application-level data.
Local variables are available in middleware via `req.app.locals` (see [req.app](#reqapp))
...
It is similar to the [baseUrl](#reqbaseurl) property of the `req` object, except `req.baseUrl`
...
For details, see [Application settings](/4x/api#application-settings).
:::
...
it is usually better to use [req.baseUrl](#reqbaseurl) to get the canonical path of the app.
...
that is an object containing local variables for the view. It is like [res.render()](#resrenderview--locals--callback),
...
:::
  ...
  <td>String</td>
  <td markdown="1">
  Environment mode. Be sure to set to "production" in a production environment; see [Production best practices: performance and reliability](/advanced/best-practice-performance).
  </td>
  <td markdown="1">
  ...
  <td>Varied</td>
  <td markdown="1">
  Set the ETag response header. For possible values, see the [`etag` options table](#options-for-etag-setting).
...
If you need to make changes to a key in `req.params`, use the [app.param](/4x/api#appparamname-callback) handler. Changes are applicable only to [parameters](/guide/routing#route-parameters) already defined in the route path.
...
:::note
When called from a middleware, the mount point is not included in `req.path`. See [app.use()](/4x/api#appusepath-callback--callback) for more details.
:::
...
Later you may access this value through the [req.signedCookie](#reqsignedcookies) object.
...
`options` is identical to those given to [res.cookie()](#rescookiename-value--options), excluding
`expires` and `maxAge`.
:::
...
It uses [req.accepts()](#reqacceptstypes) to select a handler for the request, based on the acceptable
